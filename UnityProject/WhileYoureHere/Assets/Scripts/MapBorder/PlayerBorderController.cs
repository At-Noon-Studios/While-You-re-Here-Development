...
namespace MapBorder
{
    ...
    {
        [Header("Border effect settings")] [SerializeField]
        private int distanceToTriggerEffects = 5;

        [SerializeField] private int timeBeforeRetrigger = 10;

        [Header("Audio clips")] [SerializeField]
        private AudioClip tooFarFromCabinVoiceLine;
        ...
        void Update()
        {
            ...
            if (closestBorder < distanceToTriggerEffects * distanceToTriggerEffects)
            {
                ...
                _vignette.intensity.value =
                    Mathf.Clamp(1 - closestBorder / (distanceToTriggerEffects * distanceToTriggerEffects), 0f, 0.5f);
                _colorAdjustments.saturation.value =
                    Mathf.Clamp(
                        (distanceToTriggerEffects * distanceToTriggerEffects - closestBorder) /
                        (distanceToTriggerEffects * distanceToTriggerEffects) * -100, -100, 0);
            }
            else
            {
